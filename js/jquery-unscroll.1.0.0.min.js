...
}}});};a.prototype.drag=function(){var j=this.$ele,i=this.status,f=this.controlX,o=this.controlY,m=c(document);var h=j.offset().top,k=j.offset().left,l=0,e=0,n=0,g=0;
if(f){f.on("mousedown",function(p){p=p||window.event;p.preventDefault();n=p.offsetX;i.wheelflag=false;i.mouseX=true;});}if(o){o.on("mousedown",function(p){p=p||window.event;
p.preventDefault();l=p.offsetY;i.wheelflag=false;i.mouseY=true;});}m.on("mouseup",function(p){i.mouseY=false;i.mouseX=false;i.wheelflag=true;});m.on("mousemove",function(p){if(i.mouseY){e=p.clientY;
i.moveH=e-l-h;if(i.moveH<0){i.moveH=0;}else{if(i.moveH+i.controlH>i.boxH-i.space){i.moveH=i.boxH-i.controlH-i.space;}}j.scrollExcute("y");}if(i.mouseX){g=p.clientX;
i.moveW=g-n-k;if(i.moveW<0){i.moveW=0;}else{if(i.moveW+i.controlW>i.boxW-i.space){i.moveW=i.boxW-i.controlW-i.space;}}j.scrollExcute("x");}});};a.prototype.display=function(){var g=this.$ele,e=this.status,h=this.controlX,f=this.controlY;
...
if(h.css("position")==="static"){h.css("position","relative");}if(j.axis==="yx"||j.axis==="xy"){e.space=j.width;}var f="unscroll-"+j.theme;var g=c("<div class='"+f+"-Y'></div>").addClass("scroll-controlY");
...
c.fn.unscroll=function(g,f,k,j,i){var h=c(this);c.each(h,function(l,m){var e=new d(m,g,f,k,j,i);m.scrollexcute=new b(m);e.layout();e.initEvent();});};})(jQuery);
